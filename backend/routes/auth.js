  ...
  try {
    ...
    if (!user.senha) {
      return res.status(401).json({
        success: false,
        message: "Use o login do Google para esta conta",
      });
    }
...
router.post("/google", async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      console.error(
        "Mongoose não está conectado! Estado:",
        mongoose.connection.readyState,
      );
      return res
        .status(500)
        .json({ success: false, message: "Banco de dados não conectado" });
    }

    const db = mongoose.connection.db;

    if (!db) {
      console.error("db está undefined!");
      return res
        .status(500)
        .json({ success: false, message: "Erro de conexão com banco" });
    }
    ...
    res.status(500).json({
      success: false,
      message: "Erro ao fazer login com Google: " + error.message,
    });
  }
});
...
router.get("/perfil", async (req, res) => {
  try {
    ...
    if (!token) {
      return res
        .status(401)
        .json({ success: false, message: "Nao autorizado" });
    }
